...
namespace ScePSP.Cpu.Emitter
{
    ...
    unsafe public class AstMipsGenerator : AstGenerator
    {
        ...
        static readonly FieldInfo CpuThreadState_MethodCache_FieldInfo = typeof(ScePSP.Cpu.CpuThreadState).GetField("MethodCache");
        ...
        static readonly private CpuThreadState CpuThreadStateMethods = ScePSP.Cpu.CpuThreadState.Methods;
        static readonly private Type CpuThreadStateType = typeof(ScePSP.Cpu.CpuThreadState);
        ...
        public AstNodeExpr MemoryGetPointer(PspMemory Memory, AstNodeExpr Address, bool Safe, string ErrorDescription = "ERROR", InvalidAddressAsEnum InvalidAddress = InvalidAddressAsEnum.Exception)
        {
            if (Safe)
            {
                return ast.CallInstance(
                    ast.CpuThreadState,
                    (AddressToPointerWithErrorFunc)ScePSP.Cpu.CpuThreadState.Methods.GetMemoryPtrSafeWithError,
            ...
            else
            {
                ...
                else
                {
                    return ast.CallInstance(
                        ast.CpuThreadState,
                        (AddressToPointerFunc)ScePSP.Cpu.CpuThreadState.Methods.GetMemoryPtr,
                        Address
                    );
                }
            }
