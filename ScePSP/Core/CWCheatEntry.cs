...
namespace ScePSP
{
    public struct CWCheatEntry
    {
        ...
        byte OpParam => (byte)((this.Code & 0x0F000000) >> 24);
            ...
            switch (OpCode)
            {
                case 0xD:
                    if (OpParam == 0x5)
                    {
                        ...
                        List.Dequeue(), List.Dequeue(), List.Dequeue(), List.Dequeue() };
                    }
                    else
                    {
                        Values = new[] { List.Dequeue(), List.Dequeue(), List.Dequeue() };
        ...
        private void _Patch(PspMemory PspMemory)
        {
            ...
            try
            {
                switch (OpCode)
                {
                    ...
                    case 0xD:
                        {
                            uint SubParam = this.OpParam;
                            uint CompareAddress = Address;
                            uint CompareValue = BitUtils.Extract(Values[0], 0, 16);
                            uint SkipCount = BitUtils.Extract(Values[1], 0, 16);
                            uint TargetAddress = BitUtils.Extract(Values[2], 0, 32);

                            ushort CurrentValue = PspMemory.ReadSafe<ushort>(CompareAddress);
                            bool Skip = false;
                            ...
                            switch (SubParam)
                            {
                                case 0x0:
                                    ...
                                    Skip = CurrentValue != CompareValue;
                                    break;
                                case 0x1:
                                ...
                                case 0x3:
                                    ...
                                    break;
                                case 0x5:
                                    bool multiCheck = true;
                                    for (int i = 0; i < Values.Length - 3; i++)
                                    {
                                        ushort checkVal = (ushort)BitUtils.Extract(Values[3 + i], 0, 16);
                                        if (PspMemory.ReadSafe<ushort>((uint)(CompareAddress + i * 2)) != checkVal)
                                        {
                                            multiCheck = false;
                                            break;
                                        }
                            ...
                            if (!Skip)
                            {
                                uint writeValue = BitUtils.Extract(Values[Values.Length - 1], 0, 32);
                                PspMemory.WriteSafe(TargetAddress, writeValue);
                            }
                        }
                        break;
                    default:
