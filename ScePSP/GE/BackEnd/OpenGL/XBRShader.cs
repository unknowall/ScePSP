...
namespace ScePSP.BackEnd.OpenGL.Shader
{
	public class XBRShader : IDisposable
	{
		...
		public GLTexture2D Process(GLTexture2D Input)
		{
			Filter.SetSize(Input.Width * 2, Input.Height * 2);
			return Filter.Process((_Shader) =>
			{
				_Shader.GetUniform("OGL2Texture")
					.Set(GLTextureUnit.CreateAtIndex(0)
					.SetWrap(GLWrap.ClampToEdge)
					.SetFiltering(GLScaleFilter.Nearest)
					.SetTexture(Input));
			});
		}
