...
namespace ScePSP.GE.Run
{
    public unsafe sealed partial class GERunner
    {
        ...
        {
            GpuState->VertexAddress = (Params24);
        }

        // Index List (Base Address)
        public void OP_IADDR()
        {
            GpuState->IndexAddress = (Params24);
        }
        ...
        public void OP_PRIM()
        {
            ...
            if (PrimitiveType != GuPrimitiveType.ContinuePreviousPrim) _lastPrimType = PrimitiveType;

            //Console.Out.WriteLineColored(ConsoleColor.Cyan, $"GE: {GECore.Id} PRIM: Type {PrimitiveType} VertexCount {VertexCount}");

            var NextInstruction = *(GpuInstruction*)GECore.Memory.PspAddressToPointerUnsafe(PC + 4);

            if (PrimCount == 0)
            {
                GECore.GeList.BackEnd.BeforeDraw(GECore.GEStateStruct);
                GECore.GeList.BackEnd.PrimStart(GlobalGpuState, GECore.GEStateStruct, _lastPrimType);
            }

            if (VertexCount > 0)
            {
                GECore.GeList.BackEnd.Prim(VertexCount, PrimCount);
            }

            if (NextInstruction.OpCode == OpCodes.PRIM && ((GuPrimitiveType)BitUtils.Extract(NextInstruction.Params, 16, 3) == PrimitiveType))
            {
                PrimCount++;
            }
            else
        ...
        public void OP_PPRIM()
        {
            ...
            if (PrimitiveType != GuPrimitiveType.ContinuePreviousPrim) _lastPPrimType = PrimitiveType;
