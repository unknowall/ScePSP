    ...
    public unsafe partial class EncryptedPrx
    {
        ...
        protected byte[] DecryptPRX1(byte[] _pbIn, bool ShowInfo = false)
        {
            ...
            fixed (byte* pbOut = _pbOut)
            {
                ...
                h7_header->Mode = Crypto.Kirk.KirkMode.DecryptCbc;
        ...
        protected byte[] DecryptPRX2(byte[] _pbIn, bool ShowInfo = false)
        {
            ...
            fixed (byte* tmp3 = _tmp3)
            {
                ...
                *((uint*)outbuf) = 0x014C;
                ...
                /* sha-1 */
                if (Kirk.hleUtilsBufferCopyWithRange(outbuf, 3000000, outbuf, 3000000, Crypto.Kirk.CommandEnum.PSP_KIRK_CMD_SHA1_HASH) != Crypto.Kirk.ResultEnum.OK)
                {
                ...
                *(uint*)&outbuf[0xA0] = 1;
                ...
                // the real decryption
                var ret = Kirk.hleUtilsBufferCopyWithRange(outbuf, size, outbuf + 0x40, size - 0x40, Crypto.Kirk.CommandEnum.PSP_KIRK_CMD_DECRYPT_PRIVATE);
                if (ret != 0)
                {
