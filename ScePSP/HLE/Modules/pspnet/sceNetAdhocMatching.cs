...
namespace ScePSP.Hle.Modules.pspnet
{
    ...
    public unsafe class sceNetAdhocMatching : HleModuleHost
    {
        ...
        public class Matching : IHleUidPoolClass
        {
            ...
            public Matching(PspContext PspContext)
            {
            ...
            public void NotifyEvent(Event Event, MacAddress Mac, byte[] Data)
            {
...

                {
                    ...
                    Console.WriteLine("Executing callback. Matching.NotifyEvent: 0x{0:X8}, {1}, {2}, 0x{3:X8}, {4}, 0x{5:X8}", this.Callback, this.GetUidIndex(PspContext), Event, MacPartition.Low, DataPartition.Size, DataPartition.Low);
                    HleInterop.ExecuteFunctionLater(
                        this.Callback,
                        this.GetUidIndex(PspContext),
                        (int)Event,
                        MacPartition.Low,
                        DataPartition.Size,
            ...
            {
                (HelloThread = new Thread(() => { while (Runing) { SendHello(); Thread.Sleep(HelloDelay / 1000); } }) { IsBackground = true }).Start();
                (PingThread = new Thread(() => { while (Runing) { SendPing(); Thread.Sleep(PingDelay / 1000); } }) { IsBackground = true }).Start();
        ...
        {
            //throw (new NotImplementedException());
            return new Matching(PspContext)
            {
                Mode = Mode,
                MaxPeers = MaxPeers,
                Port = Port,
