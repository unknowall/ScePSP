...
namespace LightCodec.Utils
{
    public class FFT
    {
        ...
        internal int[] revtab = new int[0];
        internal float[] tmpBuf = new float[0];
        internal int mdctSize; // size of MDCT (i.e. number of input data * 2)
        internal int mdctBits; // n = 2^nbits
        ...
        private static readonly float[] ff_cos_64 = new float[64 / 2];
        private static readonly float[] ff_cos_128 = new float[128 / 2];
        private static readonly float[] ff_cos_256 = new float[256 / 2];
        private static readonly float[] ff_cos_512 = new float[512 / 2];

        public static void copy(int[] to, int[] from)
        {
            System.Array.Copy(from, 0, to, 0, to.Length);
        }
        ...
        public virtual void copy(FFT that)
        {
            nbits = that.nbits;
        ...
        private int fftInit(int nbits, bool inverse)
        {
            if (nbits < 2 || nbits > 16)
            {
                revtab = null;
        ...
        public virtual int mdctInit(int nbits, bool inverse, double scale)
        {
            int n = 1 << nbits;
            mdctBits = nbits;
            mdctSize = n;
        ...
        public virtual void imdctCalc(float[] output, int outputOffset, float[] input, int inputOffset)
        {
            int n = 1 << mdctBits;
            int n2 = n >> 1;
            int n4 = n >> 2;
        ...
        public virtual void imdctHalf(float[] output, int outputOffset, float[] input, int inputOffset)
        {
            int n = 1 << mdctBits;
            int n2 = n >> 1;
            int n4 = n >> 2;
        ...
        public virtual void fftCalcFloat(float[] z, int o)
        {
            switch (nbits)
            {
                case 2:
                ...
                case 9:
                    ...
                    break;
                default:
                    System.Console.WriteLine(string.Format("LightCodecs FFT nbits={0:D} not implemented", nbits));
                    break;
            }
        }

        public virtual void mdctCalc(float[] output, int outputOffset, float[] input, int inputOffset)
        {
            int n = 1 << mdctBits;
            int n2 = n >> 1;
            int n4 = n >> 2;
