...
{
    using static LightCodec.aac.AacTab;
    using static LightCodec.aac.Lpc;
    using static LightCodec.aac.OutputConfiguration;
    using static LightCodec.Utils.CodecUtils;
    using static LightCodec.Utils.SineWin;
    ...
    public class AacDecoder : ILightCodec
    {
		...
		private int setDefaultChannelConfig(int[][] layoutMap, int[] tags, int channelConfig)
		{
			...
			{
				Utils.copy(layoutMap[i], aac_channel_layout_map[channelConfig - 1][i]);
			}
				...
				for (int i = 0; i < ics.maxSfb;)
				{
					...
					else if (bandType[idx] == INTENSITY_BT || bandType[idx] == INTENSITY_BT2)
					{
						for (; i < runEnd; i++, idx++)
						{
							...
							int clippedOffset = Utils.clip(offset[2], -155, 100);
					...
					else if (bandType[idx] == NOISE_BT)
					{
						for (; i < runEnd; i++, idx++)
						{
							...
							int clippedOffset = Utils.clip(offset[1], -100, 155);
			...
			if (pulsePresent)
			{
				...
				for (int i = 0; i < pulse.numPulse; i++)
				{
					...
					if (bandType[idx] != NOISE_BT && sf[idx] != 0f)
					{
						...
						coef[pulse.pos[i]] = (float) cbrt(System.Math.Abs(ico)) * ico * sf[idx];
					}
				}
			}
		...
		private int decodePce(MPEG4AudioConfig m4ac, int[][] layoutMap)
		{
			...
			// comment FieldInfo, first byte is Length
        ...
        private int decodeFill(int len)
		{
			if (len >= 13 + 7 * 8)
			{
                ...
                //Pattern p = compile("libfaac (\\d+)\\.(\\d+)");
                Regex p = compile("libfaac (\\d+)\\.(\\d+)");
		...
		public virtual unsafe int decode(void* inputAddr, int inputLength, void* output, out int outputLength)
        {
            ...
            int sampleBytes = ac.nbSamples * ac.outputChannels * sizeof(short);

            CodecUtils.writeOutput(ac.samples, (short*)output + outputLength / sizeof(short), ac.nbSamples, ac.channels, ac.outputChannels);
