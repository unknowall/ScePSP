...
{
	using static LightCodec.aac.PSContext;
	using static LightCodec.aac.AacPsData;
    using AacPsData = LightCodec.aac.AacPsData;
    ...
    public class AacPs
	{
		private const int numQMFSlots = 32; // numTimeSlots * RATE

		private static readonly int[][] numEnv_tab = new int[][]
		{
			new int[] {0, 1, 2, 4},
			new int[] {1, 2, 3, 4}
		};

		private static readonly int[] nr_iidiccPar_tab = new int[] {10, 20, 34, 10, 20, 34};

		private static readonly int[] nr_iidopdPar_tab = new int[] {5, 11, 17, 5, 11, 17};

		private const int huff_iid_df1 = 0;
		private const int huff_iid_dt1 = 1;
		private const int huff_iid_df0 = 2;
		private const int huff_iid_dt0 = 3;
		private const int huff_icc_df = 4;
		private const int huff_icc_dt = 5;
		private const int huff_ipd_df = 6;
		private const int huff_ipd_dt = 7;
		private const int huff_opd_df = 8;
		private const int huff_opd_dt = 9;

		private static readonly int[] huff_iid = new int[] {huff_iid_df0, huff_iid_df1, huff_iid_dt0, huff_iid_dt1};
		...
		{
			ps.start = false;
			int bitsRead = ac.br.BitsRead - bitCountStart;
			if (bitsRead < bitsLeft)
			{
				ac.br.skip(bitsLeft - bitsRead);
			}
			Utils.fill(ps.iidPar, 0);
			Utils.fill(ps.iccPar, 0);
			Utils.fill(ps.ipdPar, 0);
			Utils.fill(ps.opdPar, 0);

			return bitsLeft;
		}

		private static int readParData(Context ac, PSContext ps, int[][] par, int tableIdx, int e, bool dt, int num, int offset, int mask)
		{
			VLC vlc = vlc_ps[tableIdx];
			if (dt)
			{
				int ePrev = e != 0 ? e - 1 : ps.numEnvOld - 1;
				ePrev = System.Math.Max(ePrev, 0);
				for (int b = 0; b < num; b++)
				{
		...
		private static int readIidData(Context ac, PSContext ps, int[][] iid, int tableIdx, int e, bool dt)
		{
			return readParData(ac, ps, iid, tableIdx, e, dt, ps.nrIidPar, huff_offset[tableIdx], 0);
		}

		private static int readIccData(Context ac, PSContext ps, int[][] iid, int tableIdx, int e, bool dt)
		{
			return readParData(ac, ps, iid, tableIdx, e, dt, ps.nrIccPar, huff_offset[tableIdx], 0);
		}

		private static int readIpdopdData(Context ac, PSContext ps, int[][] iid, int tableIdx, int e, bool dt)
		{
		...
		private static int readExtensionData(Context ac, PSContext ps, int psExtensionId)
		{
			...
			return ac.br.BitsRead - count;
		}

		public static int readData(Context ac, PSContext ps, int bitsLeft)
		{
			BitReader br = ac.br;
			int bitCountStart = br.BitsRead;

			bool header = br.readBool();
			if (header)
			{ // enable_ps_header
			...
			if (ps.numEnv == 0 || ps.borderPosition[ps.numEnv] < numQMFSlots - 1)
			{
				...
				if (source >= 0 && source != ps.numEnv)
				{
					if (ps.enableIid)
					{
						Array.Copy(ps.iidPar[source], 0, ps.iidPar[ps.numEnv], 0, ps.iidPar[0].Length);
					}
					if (ps.enableIcc)
					{
						Array.Copy(ps.iccPar[source], 0, ps.iccPar[ps.numEnv], 0, ps.iccPar[0].Length);
					}
					if (ps.enableIpdopd)
					{
						Array.Copy(ps.ipdPar[source], 0, ps.ipdPar[ps.numEnv], 0, ps.ipdPar[0].Length);
						Array.Copy(ps.opdPar[source], 0, ps.opdPar[ps.numEnv], 0, ps.opdPar[0].Length);
					}
				}
				if (ps.enableIid)
				{
		...
		/// <summary>
		/// Split one subband into 6 subsubbands with a complex filter </summary>
		private static void hybrid6_cx(float[][] @in, float[][][] @out, int outOffset, float[][][] filter, int len)
		{
			const int N = 8;

			float[][] temp = RectangularArrays.ReturnRectangularFloatArray(8, 2);
		...
		private static void hybrid_analysis(float[][][] @out, float[][][] @in, float[][][] L, bool is34, int len)
		{
			for (int i = 0; i < 5; i++)
			{
				for (int j = 0; j < 38; j++)
			...
			for (int i = 0; i < 5; i++)
			{
				for (int j = 0; j < 6; j++)
				{
					Array.Copy(@in[i][j + 32], 0, @in[i][j], 0, @in[i][0].Length);
				}
			}
		}

		internal static void hybrid_synthesis(float[][][] @out, float[][][] @in, bool is34, int len)
		{
			if (is34)
			{
				for (int n = 0; n < len; n++)
		...
		private const float DECAY_SLOPE = 0.05f;
		/// Number of frequency bands that can be addressed by the parameter index, b(k)
		private static readonly int[] NR_PAR_BANDS = new int[] {20, 34};
		private static readonly int[] NR_IPDOPD_BANDS = new int[] {11, 17};
		/// Number of frequency bands that can be addressed by the sub subband index, k
		private static readonly int[] NR_BANDS = new int[] {71, 91};
		/// Start frequency band for the all-pass filter decay slope
		private static readonly int[] DECAY_CUTOFF = new int[] {10, 32};
		/// Number of all-pass filer bands
		private static readonly int[] NR_ALLPASS_BANDS = new int[] {30, 50};
		/// First stereo band using the short one sample delay
		private static readonly int[] SHORT_DELAY_BAND = new int[] {42, 62};

		/// <summary>
		/// Table 8.46 </summary>
		internal static void map_idx_10_to_20(int[] par_mapped, int[] par, bool full)
		{
			int b;
		...
		internal static void map_idx_34_to_20(int[] par_mapped, int[] par, bool full)
		{
		...
		internal static void map_val_34_to_20(float[] par)
		{
		...
		internal static void map_idx_10_to_34(int[] par_mapped, int[] par, bool full)
		{
			if (full)
			{
				par_mapped[33] = par[9];
		...
		internal static void map_idx_20_to_34(int[] par_mapped, int[] par, bool full)
		{
			if (full)
			{
		...
		internal static void map_val_20_to_34(float[] par)
		{
			par[33] = par[19];
			par[32] = par[19];
			par[31] = par[18];
		...
		internal static void decorrelation(PSContext ps, float[][][] @out, float[][][] s, bool is34)
		{

			int is34i = is34 ? 1 : 0;

			float[][] power = RectangularArrays.ReturnRectangularFloatArray(34, PS_QMF_TIME_SLOTS);

			float[][] transient_gain = RectangularArrays.ReturnRectangularFloatArray(34, PS_QMF_TIME_SLOTS);
			...
			int[] k_to_i = is34 ? k_to_i_34 : k_to_i_20;
			...
			{
				Utils.fill(ps.peakDecayNrg, 0f);
				Utils.fill(ps.powerSmooth, 0f);
				Utils.fill(ps.peakDecayDiffSmooth, 0f);
				Utils.fill(ps.delay, 0f);
				Utils.fill(ps.apDelay, 0f);
			}

			for (int kk = 0; kk < NR_BANDS[is34i]; kk++)
			{
				int i = k_to_i[kk];
				PSDSP.add_squares(power[i], 0, s[kk], 0, nL - n0);
			}
			...
			for (int i = 0; i < NR_PAR_BANDS[is34i]; i++)
			{
				for (int n = n0; n < nL; n++)
				{
					...
					float denom;
					peak_decay_nrg[i] = System.Math.Max(decayed_peak, power[i][n]);
			...
			{
				int b = k_to_i[k];
				float g_decay_slope = 1.0f - DECAY_SLOPE * (k - DECAY_CUTOFF[is34i]);
				g_decay_slope = Utils.clipf(g_decay_slope, 0.0f, 1.0f);
				for (int i = 0; i < PS_MAX_DELAY; i++)
				{
					Utils.copy(delay[k][i], delay[k][nL + i]);
				}
				for (int i = 0; i < numQMFSlots; i++)
				{
					Utils.copy(delay[k][PS_MAX_DELAY + i], s[k][i]);
				}
				for (int m = 0; m < PSContext.PS_AP_LINKS; m++)
				{
					for (int i = 0; i < 5; i++)
					{
						Utils.copy(ap_delay[k][m][i], ap_delay[k][m][numQMFSlots + i]);
					}
				}
				PSDSP.decorrelate(@out[k], 0, delay[k], PS_MAX_DELAY - 2, ap_delay[k], phi_fract[is34i][k], Q_fract_allpass[is34i][k], transient_gain[b], g_decay_slope, nL - n0);
			}

			for (; k < SHORT_DELAY_BAND[is34i]; k++)
			{
				for (int ii = 0; ii < PS_MAX_DELAY; ii++)
				{
					Utils.copy(delay[k][ii], delay[k][nL + ii]);
				}
				for (int iii = 0; iii < numQMFSlots; iii++)
				{
					Utils.copy(delay[k][PS_MAX_DELAY + iii], s[k][iii]);
				}
				// H = delay 14
				int i = k_to_i[k];
		...
		private static void remap34(int[][][] p_par_mapped, int[][] par, int num_par, int numEnv, bool full)
		{
		...
		private static void remap20(int[][][] p_par_mapped, int[][] par, int num_par, int numEnv, bool full)
		{
			int[][] par_mapped = p_par_mapped[0];
		...
		private static void stereo_processing(PSContext ps, float[][][] l, float[][][] r, bool is34)
		{
			int is34i = is34 ? 1 : 0;
			float[][][] H11 = ps.H11;
			float[][][] H12 = ps.H12;
			...
			int[][] iid_mapped_buf = RectangularArrays.ReturnRectangularIntArray(PS_MAX_NUM_ENV, PS_MAX_NR_IIDICC);

			int[][] icc_mapped_buf = RectangularArrays.ReturnRectangularIntArray(PS_MAX_NUM_ENV, PS_MAX_NR_IIDICC);

			int[][] ipd_mapped_buf = RectangularArrays.ReturnRectangularIntArray(PS_MAX_NUM_ENV, PS_MAX_NR_IIDICC);

			int[][] opd_mapped_buf = RectangularArrays.ReturnRectangularIntArray(PS_MAX_NUM_ENV, PS_MAX_NR_IIDICC);
			...
			float[][][] H_LUT = ps.iccMode < 3 ? HA : HB;

			// Remapping
			if (ps.numEnvOld != 0)
			{
				Array.Copy(H11[0][ps.numEnvOld], 0, H11[0][0], 0, PS_MAX_NR_IIDICC);
				Array.Copy(H11[1][ps.numEnvOld], 0, H11[1][0], 0, PS_MAX_NR_IIDICC);
				Array.Copy(H12[0][ps.numEnvOld], 0, H12[0][0], 0, PS_MAX_NR_IIDICC);
				Array.Copy(H12[1][ps.numEnvOld], 0, H12[1][0], 0, PS_MAX_NR_IIDICC);
				Array.Copy(H21[0][ps.numEnvOld], 0, H21[0][0], 0, PS_MAX_NR_IIDICC);
				Array.Copy(H21[1][ps.numEnvOld], 0, H21[1][0], 0, PS_MAX_NR_IIDICC);
				Array.Copy(H22[0][ps.numEnvOld], 0, H22[0][0], 0, PS_MAX_NR_IIDICC);
				Array.Copy(H22[1][ps.numEnvOld], 0, H22[1][0], 0, PS_MAX_NR_IIDICC);
			}
			...
			for (int e = 0; e < ps.numEnv; e++)
			{
				...
				for (int k = 0; k < NR_BANDS[is34i]; k++)
				{
					...
					float[][] h = RectangularArrays.ReturnRectangularFloatArray(2, 4);

					float[][] h_step = RectangularArrays.ReturnRectangularFloatArray(2, 4);
					int start = ps.borderPosition[e];
					int stop = ps.borderPosition[e+1];
					float width = 1.0f / (stop - start);
					int b = k_to_i[k];
					h[0][0] = H11[0][e][b];
					h[0][1] = H12[0][e][b];
					h[0][2] = H21[0][e][b];
		...
		public static int psApply(PSContext ps, float[][][] L, float[][][] R, int top)
		{
			...
			float[][][] Lbuf = RectangularArrays.ReturnRectangularFloatArray(91, 32, 2);

			float[][][] Rbuf = RectangularArrays.ReturnRectangularFloatArray(91, 32, 2);
			const int len = 32;

			bool is34 = ps.is34bands;
