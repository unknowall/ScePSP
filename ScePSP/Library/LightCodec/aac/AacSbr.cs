...
namespace LightCodec.aac
{
    ...
    using static LightCodec.aac.SBRData;
	...
	case 192000:
		...
		private static int sbrHfCalcNpatches(Context ac, SpectralBandReplication sbr)
		{
			...
			do
			{
				...
				// After this check the sealed override number of patches can still be six which is
				// illegal however the Coding Technologies decoder check stream has a sealed override
				// count of 6 patches
				if (sbr.numPatches > 5)
				{
