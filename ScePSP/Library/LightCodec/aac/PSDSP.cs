...
namespace LightCodec.aac
{
	...
	public class PSDSP
	{
		...
		public static void stereoInterpolate(float[][] l, int lOffset, float[][] r, int rOffset, float[][] h, float[][] h_step, int len, bool ipdopd)
		{
			if (ipdopd)
			{
		...
		public static void add_squares(float[] dst, int dstOffset, float[][] src, int srcOffset, int n)
		{
			for (int i = 0; i < n; i++)
			{
		...
		public static void hybrid_analysis(float[][] @out, int outOffset, float[][] @in, int inOffset, float[][][] filter, int filterOffset, int stride, int n)
		{
			for (int i = 0; i < n; i++)
			{
				...
				for (int j = 0; j < 6; j++)
				{
					...
					sum_re += filter[filterOffset + i][j][0] * (in0_re + in1_re) - filter[filterOffset + i][j][1] * (in0_im - in1_im);
					sum_im += filter[filterOffset + i][j][0] * (in0_im + in1_im) + filter[filterOffset + i][j][1] * (in0_re - in1_re);
				}
		...
		private static readonly float[] a = new float[] {0.65143905753106f, 0.56471812200776f, 0.48954165955695f};
