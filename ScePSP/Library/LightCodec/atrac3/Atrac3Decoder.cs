...
namespace LightCodec.atrac3
{
    public class Atrac3Decoder : ILightCodec
    {
        ...
        private const int MDCT_SIZE = 512;
        private static readonly float[] mdct_window = new float[MDCT_SIZE];
        private static readonly VLC[] spectral_coeff_tab = new VLC[7];
        private Context ctx;
        ...
        public virtual int init(int blockAlign, int channels, int outputChannels, int codingMode)
        {
            int ret;
        ...
        private void imlt(float[] input, int inputOffset, float[] output, int outputOffset, bool oddBand)
        {
            if (oddBand)
            {
        ...
        /// </summary>
        /// <param name="components"> tonal components </param>
        /// <param name="numBands">   number of coded bands </param>
        ...
        /// </summary>
        /// <returns> subband count, fix for broken specification/files </returns>
        private int decodeSpectrum(float[] output)
        {
            ...
            for (int x = 0; x <= numSubbands; x++)
            {
                subbandVlcIndex[x] = br.read(3);
            }

            // read the scale factor indexes from the stream
            for (int x = 0; x <= numSubbands; x++)
            {
                if (subbandVlcIndex[x] != 0)
                {
                    sfIndex[x] = br.read(6);
                }
            }
        ...
        /// </summary>
        /// <param name="spectrum">        output spectrum buffer </param>
        /// <param name="numComponents">   number of tonal components </param>
        /// <param name="components">      tonal components for this band </param>
        /// <returns>                position of the last tonal coefficient </returns>
        private int addTonalComponents(float[] spectrum, int numComponents, TonalComponent[] components)
        {
            int lastPos = -1;
            for (int i = 0; i < numComponents; i++)
            {
                lastPos = System.Math.Max(components[i].pos + components[i].numCoefs, lastPos);
            ...
            for (int i = 0, band = 0; band < 4 * 256; band += 256, i++)
            {
                ...
                if (s1 != s2)
                {
                    ...
                    for (; nsample < band + 8; nsample++)
                    {
                        ...
                        c2 = c1 * INTERPOLATE(mc1l, mc2l, nsample - band) + c2 * INTERPOLATE(mc1r, mc2r, nsample - band);
                        su1[nsample] = c2;
        ...
        {
            float[][] w = RectangularArrays.ReturnRectangularFloatArray(2, 2);
