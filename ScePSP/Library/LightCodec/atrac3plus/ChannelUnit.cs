...
namespace LightCodec.atrac3plus
{
    public class ChannelUnit
    {
        ...
        public virtual BitReader BitReader
        {
            set
            {
                this.br = value;
        ...
        public virtual Atrac3plusDsp Dsp
        {
            set
            {
                this.dsp = value;
        ...
        /// 
        /// @param[in,out] chan          ptr to the channel parameters </summary>
        ...
        /// 
        /// @param[in,out] chan          ptr to the channel parameters
        /// @param[in]     wtab_idx      index of table of weights </summary>
        /// <returns> result code: 0 = OK, otherwise - error code </returns>
            ...
            for (int chNum = 0; chNum < numChannels; chNum++)
            {
                ...
                for (int qu = 0; qu < ctx.usedQuantUnits; qu++)
                {
                    ...
                    else if (chNum > 0 && ctx.channels[0].quWordlen[qu] != 0 && codetab == 0)
                    {
                        ...
                        Array.Copy(ctx.channels[0].spectrum, ff_atrac3p_qu_to_spec_pos[qu], chan.spectrum, ff_atrac3p_qu_to_spec_pos[qu], numSpecs);
        ...
        private bool getSubbandFlags(bool[] @out, int numFlags)
        {
            bool result = br.readBool();
            if (result)
            {
                if (br.readBool())
                {
        ...
        /// <summary>
        /// : coding mode 1 (master) for gain compensation levels.
        /// 
        ...
        /// <summary>
        /// : coding mode 3 (slave) for gain compensation levels.
        /// 
        /// @param[out]   dst   ptr to the output array
        ...
        /// @param[in]     coded_subbands  number of subbands to process </summary>
        /// <returns> result code: 0 = OK, otherwise - error code </returns>
        private int decodeGaincLevels(int chNum, int codedSubbands)
        {
            ...
            { // switch according to coding mode
                ...
                case 1:
                    if (chNum > 0)
                    { // VLC module delta to master channel
                        for (int sb = 0; sb < codedSubbands; sb++)
                        {
                            for (int i = 0; i < chan.gainData[sb].numPoints; i++)
                            {
        ...
        /// <summary>
        /// : coding mode 0 for gain compensation locations.
        /// 
        /// @param[out]    dst    ptr to the output array
        ...
        /// <summary>
        /// : coding mode 1 for gain compensation locations.
        /// 
        /// @param[out]    dst    ptr to the output array
        /// </summary>
        ...
        /// @param[in]     codedSubbands  number of subbands to process </summary>
        /// <returns> result code: 0 = OK, otherwise - error code </returns>
                ...
                case 1:
                    if (chNum > 0)
                    {
                        for (int sb = 0; sb < codedSubbands; sb++)
                        {
                            ...
                            for (int i = 1; i < dst.numPoints; i++)
                            {
                                bool moreThanRef = i >= @ref.numPoints;
                ...
                case 2:
                    ...
                    else
                    {
                        ...
                        for (int sb = 1; sb < codedSubbands; sb++)
                        {
                            ...
                            for (int i = 1; i < dst.numPoints; i++)
                            {
                                bool moreThanRef = i >= chan.gainData[sb - 1].numPoints;
        ...
        ///                                1 - tone data present
        /// </summary>
        private void decodeTonesEnvelope(int chNum, bool[] bandHasTones)
        {
        ...
        /// @param[in]     chNum           channel to process
        /// @param[in]     bandHasTones    ptr to an array of per-band-flags:
        ///                                1 - tone data present </summary>
        /// <returns> result code: 0 = OK, otherwise - error code </returns>
        private int decodeBandNumwavs(int chNum, bool[] bandHasTones)
        {
            WavesData[] dst = ctx.channels[chNum].tonesInfo;
            WavesData[] @ref = ctx.channels[0].tonesInfo;
        ...
        ///                                1 - tone data present
        /// </summary>
        private void decodeTonesFrequency(int chNum, bool[] bandHasTones)
        {
            WavesData[] dst = ctx.channels[chNum].tonesInfo;
            WavesData[] @ref = ctx.channels[0].tonesInfo;
            ...
            { // mode 0: fixed-Length coding
                for (int sb = 0; sb < ctx.wavesInfo.numToneBands; sb++)
                {
                    ...
                    bool direction = (dst[sb].numWavs > 1 ? br.readBool() : false);
                    if (direction)
            ...
            { // mode 1: VLC module delta to master (slave only)
                for (int sb = 0; sb < ctx.wavesInfo.numToneBands; sb++)
                {
                    if (!bandHasTones[sb] || dst[sb].numWavs == 0)
                    {
        ...
        ///                                1 - tone data present
        /// </summary>
        private void decodeTonesAmplitude(int chNum, bool[] bandHasTones)
        {
            WavesData[] dst = ctx.channels[chNum].tonesInfo;
            WavesData[] @ref = ctx.channels[0].tonesInfo;
            ...
            if (chNum > 0)
            {
                for (int sb = 0; sb < ctx.wavesInfo.numToneBands; sb++)
                {
                    ...
                    for (int j = 0; j < dst[sb].numWavs; j++)
                    {
                        ...
                        {
                            int diff = Math.Abs(ctx.wavesInfo.waves[wsrc + j].freqIndex - ctx.wavesInfo.waves[wref + i].freqIndex);
                            if (diff < maxdiff)
                            {
                                maxdiff = diff;
                                fi = i;
        ...
        ///                                1 - tone data present
        /// </summary>
        private void decodeTonesPhase(int chNum, bool[] bandHasTones)
        {
            WavesData[] dst = ctx.channels[chNum].tonesInfo;
        ...
        public virtual void decodeResidualSpectrum(float[][] @out)
        {
            int[] sbRNGindex = new int[ATRAC3P_SUBBANDS];

            if (ctx.muteFlag)
            {
                for (int ch = 0; ch < numChannels; ch++)
                {
                    Array.Fill(@out[ch], 0f);
                }
                return;
            }
            ...
            {
                float[] tmp = new float[ATRAC3P_SUBBAND_SAMPLES];
                for (int sb = 0; sb < ctx.numCodedSubbands; sb++)
                {
                    if (ctx.swapChannels[sb])
                    {
                        // Swap both channels
                        Array.Copy(@out[0], sb * ATRAC3P_SUBBAND_SAMPLES, tmp, 0, ATRAC3P_SUBBAND_SAMPLES);
                        Array.Copy(@out[1], sb * ATRAC3P_SUBBAND_SAMPLES, @out[0], sb * ATRAC3P_SUBBAND_SAMPLES, ATRAC3P_SUBBAND_SAMPLES);
                        Array.Copy(tmp, 0, @out[1], sb * ATRAC3P_SUBBAND_SAMPLES, ATRAC3P_SUBBAND_SAMPLES);
                    }
        ...
        public virtual void reconstructFrame(Context at3pContext)
        {
            for (int ch = 0; ch < numChannels; ch++)
            {
            ...
            for (int ch = 0; ch < numChannels; ch++)
            {
                bool[] tmp1 = ctx.channels[ch].wndShape;
