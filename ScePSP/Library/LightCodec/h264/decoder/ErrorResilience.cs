...
namespace LightCodec.h264.decoder
{
    ...
    {
        public const int MV_FROZEN = 3;
        public const int MV_CHANGED = 2;
        public const int MV_UNCHANGED = 1;

        public const int MV_TYPE_16X16 = 0;
        public const int MV_TYPE_8X8 = 1;
        public const int MV_TYPE_16X8 = 2;
        public const int MV_TYPE_FIELD = 3;
        public const int MV_TYPE_DMV = 4;
        ...
        public static void decode_mb(MpegEncContext s, int @ref)
        {
            ...
            if (true && s.codec_id == H264PredictionContext.CODEC_ID_H264)
            {
                ...
                ) //FIXME it is posible albeit uncommon that slice references differ between slices, we take the easy approuch and ignore it for now. If this turns out to have any relevance in practice then correct remapping should be added
                    @ref = 0;
                ...
                Rectangle.fill_rectangle_mv_cache(h.mv_cache[0], H264Context.scan8[0], 4, 4, 8,
                    H264Context.pack16to32(mv[0][0][0], mv[0][0][1]), 4);
                ////assert(!FRAME_MBAFF);
                ...
                for (b_x = 0; b_x < w - 1; b_x++)
                {
                    ...
                    int[] left_mv =
                        s.current_picture.motion_val_base[0][
                            s.current_picture.motion_val_offset[0] + mvy_stride * b_y + mvx_stride * b_x];
                    int[] right_mv = s.current_picture.motion_val_base[0][
                        s.current_picture.motion_val_offset[0] + mvy_stride * b_y + mvx_stride * (b_x + 1)];
                    ...
                    for (y = 0; y < 8; y++)
                    {
                        ...
                        a = dst_base[dst_offset + offset + 7 + y * stride] -
                            dst_base[dst_offset + offset + 6 + y * stride];
                        b = dst_base[dst_offset + offset + 8 + y * stride] -
                            dst_base[dst_offset + offset + 7 + y * stride];
                        c = dst_base[dst_offset + offset + 9 + y * stride] -
                            dst_base[dst_offset + offset + 8 + y * stride];
            ...
            for (b_y = 0; b_y < h - 1; b_y++)
            {
                for (b_x = 0; b_x < w; b_x++)
                {
                    ...
                    int[] top_mv =
                        s.current_picture.motion_val_base[0][
                            s.current_picture.motion_val_offset[0] + mvy_stride * b_y + mvx_stride * b_x];
                    int[] bottom_mv = s.current_picture.motion_val_base[0][
                        s.current_picture.motion_val_offset[0] + mvy_stride * (b_y + 1) + mvx_stride * b_x];
                    ...
                    for (x = 0; x < 8; x++)
                    {
                        ...
                        a = dst_base[dst_offset + offset + x + 7 * stride] -
                            dst_base[dst_offset + offset + x + 6 * stride];
                        b = dst_base[dst_offset + offset + x + 8 * stride] -
                            dst_base[dst_offset + offset + x + 7 * stride];
                        c = dst_base[dst_offset + offset + x + 9 * stride] -
                            dst_base[dst_offset + offset + x + 8 * stride];
                ...
                for (pass = 0; (changed != 0 || pass < 2) && pass < 10; pass++)
                {
                    ...
                    for (mb_y = 0; mb_y < s.mb_height; mb_y++)
                    {
                        for (mb_x = 0; mb_x < s.mb_width; mb_x++)
                        {
                            ...
                            int prev_x =
                                s.current_picture.motion_val_base[0][s.current_picture.motion_val_offset[0] + mot_index]
                                    [0];
                            int prev_y =
                                s.current_picture.motion_val_base[0][s.current_picture.motion_val_offset[0] + mot_index]
                                    [1];
                            ...
                            if (mb_x > 0 && @fixed[mb_xy - 1] != 0)
                            {
                                mv_predictor[pred_count][0] =
                                    s.current_picture.motion_val_base[0][
                                        s.current_picture.motion_val_offset[0] + mot_index - mot_step][0];
                                mv_predictor[pred_count][1] =
                                    s.current_picture.motion_val_base[0][
                                        s.current_picture.motion_val_offset[0] + mot_index - mot_step][1];
                            ...
                            if (mb_y > 0 && @fixed[mb_xy - mb_stride] != 0)
                            {
                                mv_predictor[pred_count][0] =
                                    s.current_picture.motion_val_base[0][
                                        s.current_picture.motion_val_offset[0] + mot_index - mot_stride * mot_step][0];
                                mv_predictor[pred_count][1] =
                                    s.current_picture.motion_val_base[0][
                                        s.current_picture.motion_val_offset[0] + mot_index - mot_stride * mot_step][1];
                            ...
                            for (j = 0; j < pred_count; j++)
                            {
                                ...
                                if (mb_x > 0 && @fixed[mb_xy - 1] != 0)
                                {
                                    int k;
                                    for (k = 0; k < 16; k++)
                                        score += Math.Abs(src_base[src_offset + k * s.linesize - 1] -
                                                          src_base[src_offset + k * s.linesize]);
                                }
                                if (mb_x + 1 < mb_width && @fixed[mb_xy + 1] != 0)
                                {
                                    int k;
                                    for (k = 0; k < 16; k++)
                                        score += Math.Abs(src_base[src_offset + k * s.linesize + 15] -
                                                          src_base[src_offset + k * s.linesize + 16]);
                                }
                                if (mb_y > 0 && @fixed[mb_xy - mb_stride] != 0)
                                {
                                    int k;
                                    for (k = 0; k < 16; k++)
                                        score += Math.Abs(src_base[src_offset + k - s.linesize] -
                                                          src_base[src_offset + k]);
                                }
                                if (mb_y + 1 < mb_height && @fixed[mb_xy + mb_stride] != 0)
                                {
                                    int k;
        ...
        public static int is_intra_more_likely(MpegEncContext s)
        {
            ...
            if (null == s.last_picture_ptr || null == s.last_picture_ptr.data_base[0])
            ...
            for (mb_y = 0; mb_y < s.mb_height - 1; mb_y++)
            {
                for (mb_x = 0; mb_x < s.mb_width; mb_x++)
                {
                    ...
                    if (s.pict_type == H264Context.FF_I_TYPE)
                    {
                        ...
                        is_intra_likely += pix_abs16_c(null, last_mb_ptr_base, last_mb_ptr_offset, mb_ptr_base,
                            mb_ptr_offset, s.linesize, 16);
                        is_intra_likely -= pix_abs16_c(null, last_mb_ptr_base, last_mb_ptr_offset, last_mb_ptr_base,
                            last_mb_ptr_offset + s.linesize * 16, s.linesize, 16);
                    }
                    else
                    {
                        if (0 != (7 & (int)s.current_picture.mb_type_base[s.current_picture.mb_type_offset + mb_xy]))
        ...
        public static void ff_er_frame_start(MpegEncContext s)
        {
            ...
            Arrays.Fill(s.error_status_table, 0, s.mb_stride * s.mb_height,
                MpegEncContext.MV_ERROR | MpegEncContext.AC_ERROR | MpegEncContext.DC_ERROR | MpegEncContext.VP_START |
        ...
        public static void ff_er_add_slice(MpegEncContext s, int startx, int starty, int endx, int endy, int status)
        {
            ...
            if (start_xy > 0 && skip_top * s.mb_width < start_i)
            {
                ...
                if (prev_status != (MpegEncContext.MV_END | MpegEncContext.DC_END | MpegEncContext.AC_END))
                    s.error_count = int.MaxValue;
            }
        }
        ...
        public static void ff_er_frame_end(MpegEncContext s)
        {
            ...
            if (0 == s.error_recognition || s.error_count == 0 || 0 != s.lowres ||
                s.picture_structure !=
                Constants.PICT_FRAME || // we dont support ER of field pictures yet, though it should not crash if enabled
            ...
            */
            ...
            if (0 != s.partitioned_frame)
            {
                ...
                for (i = s.mb_num - 1; i >= 0; i--)
                {
                    ...
                    if (0 != (error & MpegEncContext.AC_END))
                        end_ok = 0;
                    if (0 != (error & MpegEncContext.MV_END) || 0 != (error & MpegEncContext.DC_END) ||
                        0 != (error & MpegEncContext.AC_ERROR))
                        end_ok = 1;
            ...
            if (s.error_recognition >= 4)
            {
                ...
                for (i = s.mb_num - 2; i >= s.mb_width + 100; i--)
                {
                    ...
                    if (error2 == (MpegEncContext.VP_START | MpegEncContext.DC_ERROR | MpegEncContext.AC_ERROR |
                                   MpegEncContext.MV_ERROR | MpegEncContext.AC_END | MpegEncContext.DC_END |
                                   MpegEncContext.MV_END)
                        && error1 != (MpegEncContext.VP_START | MpegEncContext.DC_ERROR | MpegEncContext.AC_ERROR |
                                      MpegEncContext.MV_ERROR | MpegEncContext.AC_END | MpegEncContext.DC_END |
                                      MpegEncContext.MV_END)
                        && (0 != (error1 & MpegEncContext.AC_END) || 0 != (error1 & MpegEncContext.DC_END) ||
                            0 != (error1 & MpegEncContext.MV_END)))
                    {
                        //end & uninit
                        end_ok = 0;
                    }
            ...
            for (i = 0; i < s.mb_num; i++)
            {
                ...
                if (0 != is_intra_likely)
                    ...
                    s.current_picture.mb_type_base[s.current_picture.mb_type_offset + mb_xy] =
                        H264Context.MB_TYPE_16x16 | H264Context.MB_TYPE_L0;
            }
            ...
            for (mb_y = 0; mb_y < s.mb_height; mb_y++)
            {
                for (mb_x = 0; mb_x < s.mb_width; mb_x++)
                {
                    ...
                    if (0 != (mb_type & H264Context.MB_TYPE_8x8))
                    {
                        ...
                        for (j = 0; j < 4; j++)
                        {
                            mv[0][j][0] = s.current_picture.motion_val_base[dir][
                                s.current_picture.motion_val_offset[dir] + mb_index + (j & 1) +
                                (j >> 1) * s.b8_stride][0];
                            mv[0][j][1] = s.current_picture.motion_val_base[dir][
                                s.current_picture.motion_val_offset[dir] + mb_index + (j & 1) +
                                (j >> 1) * s.b8_stride][1];
                        }
                    }
                    else
                    {
                        s.mv_type = MV_TYPE_16X16;
                        mv[0][0][0] =
                            s.current_picture.motion_val_base[dir][
                                s.current_picture.motion_val_offset[dir] + mb_x * 2 + mb_y * 2 * s.b8_stride][0];
                        mv[0][0][1] =
                            s.current_picture.motion_val_base[dir][
                                s.current_picture.motion_val_offset[dir] + mb_x * 2 + mb_y * 2 * s.b8_stride][1];
                    }
                ...
                for (mb_y = 0; mb_y < s.mb_height; mb_y++)
                {
                    for (mb_x = 0; mb_x < s.mb_width; mb_x++)
                    {
                        ...
                        if (s.pp_time != 0)
                        {
                            ...
                            mv[0][0][0] =
                                s.next_picture.motion_val_base[0][s.next_picture.motion_val_offset[0] + xy][0] *
                                time_pb / time_pp;
                            mv[0][0][1] =
                                s.next_picture.motion_val_base[0][s.next_picture.motion_val_offset[0] + xy][1] *
                                time_pb / time_pp;
                            mv[1][0][0] =
                                s.next_picture.motion_val_base[0][s.next_picture.motion_val_offset[0] + xy][0] *
            ...
            for (mb_y = 0; mb_y < s.mb_height; mb_y++)
            {
                for (mb_x = 0; mb_x < s.mb_width; mb_x++)
                {
                    ...
                    put_dc(s, dest_y_base, dest_y_offset, dest_cb_base, dest_cb_offset, dest_cr_base, dest_cr_offset,
                        mb_x, mb_y);
                }
            }
            ...
            if (0 != (error_concealment & FF_EC_DEBLOCK))
            {
                ...
                h_block_filter(s, s.current_picture.data_base[0], s.current_picture.data_offset[0], s.mb_width * 2,
                    s.mb_height * 2, s.linesize, 1);
                h_block_filter(s, s.current_picture.data_base[1], s.current_picture.data_offset[1], s.mb_width,
                    s.mb_height, s.uvlinesize, 0);
                h_block_filter(s, s.current_picture.data_base[2], s.current_picture.data_offset[2], s.mb_width,
                    s.mb_height, s.uvlinesize, 0);

                // filter vertical block boundaries 
                v_block_filter(s, s.current_picture.data_base[0], s.current_picture.data_offset[0], s.mb_width * 2,
                    s.mb_height * 2, s.linesize, 1);
                v_block_filter(s, s.current_picture.data_base[1], s.current_picture.data_offset[1], s.mb_width,
                    s.mb_height, s.uvlinesize, 0);
                v_block_filter(s, s.current_picture.data_base[2], s.current_picture.data_offset[2], s.mb_width,
                    s.mb_height, s.uvlinesize, 0);
            }
            ...
            for (i = 0; i < s.mb_num; i++)
            {
                ...
                if (s.pict_type != H264Context.FF_B_TYPE && 0 !=
                    (error & (MpegEncContext.DC_ERROR | MpegEncContext.MV_ERROR | MpegEncContext.AC_ERROR)))
                {
