...
namespace LightCodec.h264.decoder
{
    public class H264DSPContext
    {
        ...
        public static void weight_h264_pixels_c(int W, int H, byte[] block, int _block_offset, int stride,
        ...
        public static void biweight_h264_pixels_c(int W, int H, byte[] dst, int _dst_offset, byte[] src,
            int _src_offset, int stride, int log2_denom, int weightd, int weights, int offset)
        {
            int y;
            ...
            {
                dst[dst_offset + 0] =
                    av_clip_uint8((src[src_offset + 0] * weights + dst[dst_offset + 0] * weightd + offset) >>
                                  (log2_denom + 1));
                dst[dst_offset + 1] =
                    av_clip_uint8((src[src_offset + 1] * weights + dst[dst_offset + 1] * weightd + offset) >>
                                  (log2_denom + 1));
                if (W == 2) continue;
                dst[dst_offset + 2] =
                    av_clip_uint8((src[src_offset + 2] * weights + dst[dst_offset + 2] * weightd + offset) >>
                                  (log2_denom + 1));
                dst[dst_offset + 3] =
                    av_clip_uint8((src[src_offset + 3] * weights + dst[dst_offset + 3] * weightd + offset) >>
                                  (log2_denom + 1));
                if (W == 4) continue;
                dst[dst_offset + 4] =
                    av_clip_uint8((src[src_offset + 4] * weights + dst[dst_offset + 4] * weightd + offset) >>
                                  (log2_denom + 1));
                dst[dst_offset + 5] =
                    av_clip_uint8((src[src_offset + 5] * weights + dst[dst_offset + 5] * weightd + offset) >>
                                  (log2_denom + 1));
                dst[dst_offset + 6] =
                    av_clip_uint8((src[src_offset + 6] * weights + dst[dst_offset + 6] * weightd + offset) >>
                                  (log2_denom + 1));
                dst[dst_offset + 7] =
                    av_clip_uint8((src[src_offset + 7] * weights + dst[dst_offset + 7] * weightd + offset) >>
                                  (log2_denom + 1));
                if (W == 8) continue;
                dst[dst_offset + 8] =
                    av_clip_uint8((src[src_offset + 8] * weights + dst[dst_offset + 8] * weightd + offset) >>
                                  (log2_denom + 1));
                dst[dst_offset + 9] =
                    av_clip_uint8((src[src_offset + 9] * weights + dst[dst_offset + 9] * weightd + offset) >>
                                  (log2_denom + 1));
                dst[dst_offset + 10] =
                    av_clip_uint8((src[src_offset + 10] * weights + dst[dst_offset + 10] * weightd + offset) >>
                                  (log2_denom + 1));
                dst[dst_offset + 11] =
                    av_clip_uint8((src[src_offset + 11] * weights + dst[dst_offset + 11] * weightd + offset) >>
                                  (log2_denom + 1));
                dst[dst_offset + 12] =
                    av_clip_uint8((src[src_offset + 12] * weights + dst[dst_offset + 12] * weightd + offset) >>
                                  (log2_denom + 1));
                dst[dst_offset + 13] =
                    av_clip_uint8((src[src_offset + 13] * weights + dst[dst_offset + 13] * weightd + offset) >>
                                  (log2_denom + 1));
                dst[dst_offset + 14] =
                    av_clip_uint8((src[src_offset + 14] * weights + dst[dst_offset + 14] * weightd + offset) >>
                                  (log2_denom + 1));
                dst[dst_offset + 15] =
                    av_clip_uint8((src[src_offset + 15] * weights + dst[dst_offset + 15] * weightd + offset) >>
                                  (log2_denom + 1));
            }
        }

        public delegate void IH264WeightFunctionStub(byte[] block, int block_offset, int stride, int log2_denom,
            int weight, int offset);

        public delegate void IH264BiWeightFunctionStub(byte[] dst, int dst_offset, byte[] src, int src_offset,
            int stride, int log2_denom, int weightd, int weights, int offset);
        ...
        public IH264BiWeightFunctionStub[] biweight_h264_pixels_tab = new IH264BiWeightFunctionStub[]
        {
            ...
            {
                biweight_h264_pixels_c(16, 16, dst, dst_offset, src, src_offset, stride, log2_denom, weightd, weights,
                    offset);
            },
            (dst, dst_offset, src, src_offset, stride, log2_denom, weightd, weights, offset) =>
            {
                biweight_h264_pixels_c(16, 8, dst, dst_offset, src, src_offset, stride, log2_denom, weightd, weights,
                    offset);
            },
            (dst, dst_offset, src, src_offset, stride, log2_denom, weightd, weights, offset) =>
            {
                biweight_h264_pixels_c(8, 16, dst, dst_offset, src, src_offset, stride, log2_denom, weightd, weights,
                    offset);
            },
            (dst, dst_offset, src, src_offset, stride, log2_denom, weightd, weights, offset) =>
            {
                biweight_h264_pixels_c(8, 8, dst, dst_offset, src, src_offset, stride, log2_denom, weightd, weights,
                    offset);
            },
            (dst, dst_offset, src, src_offset, stride, log2_denom, weightd, weights, offset) =>
            {
                biweight_h264_pixels_c(8, 4, dst, dst_offset, src, src_offset, stride, log2_denom, weightd, weights,
                    offset);
            },
            (dst, dst_offset, src, src_offset, stride, log2_denom, weightd, weights, offset) =>
            {
                biweight_h264_pixels_c(4, 8, dst, dst_offset, src, src_offset, stride, log2_denom, weightd, weights,
                    offset);
            },
            (dst, dst_offset, src, src_offset, stride, log2_denom, weightd, weights, offset) =>
            {
                biweight_h264_pixels_c(4, 4, dst, dst_offset, src, src_offset, stride, log2_denom, weightd, weights,
                    offset);
            },
            (dst, dst_offset, src, src_offset, stride, log2_denom, weightd, weights, offset) =>
            {
                biweight_h264_pixels_c(4, 2, dst, dst_offset, src, src_offset, stride, log2_denom, weightd, weights,
                    offset);
            },
            (dst, dst_offset, src, src_offset, stride, log2_denom, weightd, weights, offset) =>
            {
                biweight_h264_pixels_c(2, 4, dst, dst_offset, src, src_offset, stride, log2_denom, weightd, weights,
                    offset);
            },
            (dst, dst_offset, src, src_offset, stride, log2_denom, weightd, weights, offset) =>
            {
                biweight_h264_pixels_c(2, 2, dst, dst_offset, src, src_offset, stride, log2_denom, weightd, weights,
                    offset);
            },
        };
        ...
        /* loop filter */
        private void h264_loop_filter_luma_c(byte[] pix, int _pix_offset, int xstride, int ystride, int alpha, int beta,
            int[] tc0)
        {
            int i, d;
        ...
        public void h264_v_loop_filter_luma(byte[] pix_base /*align 16*/, int pix_offset, int stride, int alpha,
            int beta, int[] tc0)
        {
            h264_loop_filter_luma_c(pix_base, pix_offset, stride, 1, alpha, beta, tc0);
        }

        public void h264_h_loop_filter_luma(byte[] pix_base /*align 4 */, int pix_offset, int stride, int alpha,
            int beta, int[] tc0)
        {
            h264_loop_filter_luma_c(pix_base, pix_offset, 1, stride, alpha, beta, tc0);
        }

        private void h264_loop_filter_luma_intra_c(byte[] pix, int _pix_offset, int xstride, int ystride, int alpha,
            int beta)
        {
            int d;
            int pix_offset = _pix_offset;
        ...
        private void h264_loop_filter_chroma_c(byte[] pix, int _pix_offset, int xstride, int ystride, int alpha,
            int beta, int[] tc0)
        {
            int i, d;
            int pix_offset = _pix_offset;
        ...
        public void h264_v_loop_filter_chroma(byte[] pix /*align 8*/, int pix_offset, int stride, int alpha, int beta,
            int[] tc0)
        {
            h264_loop_filter_chroma_c(pix, pix_offset, stride, 1, alpha, beta, tc0);
        }

        public void h264_h_loop_filter_chroma(byte[] pix /*align 4*/, int pix_offset, int stride, int alpha, int beta,
            int[] tc0)
        {
            h264_loop_filter_chroma_c(pix, pix_offset, 1, stride, alpha, beta, tc0);
        }

        private void h264_loop_filter_chroma_intra_c(byte[] pix, int _pix_offset, int xstride, int ystride, int alpha,
            int beta)
        {
            int d;
            int pix_offset = _pix_offset;
        ...
        public void h264_v_loop_filter_chroma_intra(byte[] pix /*align 8*/, int pix_offset, int stride, int alpha,
            int beta)
        {
            h264_loop_filter_chroma_intra_c(pix, pix_offset, stride, 1, alpha, beta);
        }

        public void h264_h_loop_filter_chroma_intra(byte[] pix /*align 8*/, int pix_offset, int stride, int alpha,
            int beta)
        {
        ...
        /* IDCT */
        public void h264_idct_add(byte[] dst /*align 4*/, int offset, short[] block /*align 16*/, int block_offset,
            int stride)
        {
            idct_internal(dst, offset, block, block_offset, stride, 4, 6, 1);
        }

        public void idct_internal(byte[] dst /*align 4*/, int offset, short[] block /*align 16*/, int block_offset,
            int stride, int block_stride, int shift, int add)
        {
            int i;
        ...
        public void h264_idct8_add(byte[] dst /*align 8*/, int offset, short[] block /*align 16*/, int block_offset,
            int stride)
        {
            int i;
            int cm_pos = MAX_NEG_CROP;
            ...
            for (i = 0; i < 8; i++)
            {
                ...
                int a1 = -block[block_offset + i + 3 * 8] + block[block_offset + i + 5 * 8] -
                         block[block_offset + i + 7 * 8] - (block[block_offset + i + 7 * 8] >> 1);
                int a3 = block[block_offset + i + 1 * 8] + block[block_offset + i + 7 * 8] -
                         block[block_offset + i + 3 * 8] - (block[block_offset + i + 3 * 8] >> 1);
                int a5 = -block[block_offset + i + 1 * 8] + block[block_offset + i + 7 * 8] +
                         block[block_offset + i + 5 * 8] + (block[block_offset + i + 5 * 8] >> 1);
                int a7 = block[block_offset + i + 3 * 8] + block[block_offset + i + 5 * 8] +
                         block[block_offset + i + 1 * 8] + (block[block_offset + i + 1 * 8] >> 1);
            ...
            for (i = 0; i < 8; i++)
            {
                ...
                int a1 = -block[block_offset + 3 + i * 8] + block[block_offset + 5 + i * 8] -
                         block[block_offset + 7 + i * 8] - (block[block_offset + 7 + i * 8] >> 1);
                int a3 = block[block_offset + 1 + i * 8] + block[block_offset + 7 + i * 8] -
                         block[block_offset + 3 + i * 8] - (block[block_offset + 3 + i * 8] >> 1);
                int a5 = -block[block_offset + 1 + i * 8] + block[block_offset + 7 + i * 8] +
                         block[block_offset + 5 + i * 8] + (block[block_offset + 5 + i * 8] >> 1);
                int a7 = block[block_offset + 3 + i * 8] + block[block_offset + 5 + i * 8] +
                         block[block_offset + 1 + i * 8] + (block[block_offset + 1 + i * 8] >> 1);
        ...
        public void h264_idct_dc_add(byte[] dst /*align 4*/, int offset, short[] block /*align 16*/, int block_offset,
            int stride)
        {
            int i, j;
        ...
        public void h264_idct8_dc_add(byte[] dst /*align 8*/, int offset, short[] block /*align 16*/, int block_offset,
            int stride)
        {
            int i, j;
            int dc = (block[block_offset + 0] + 32) >> 6;
        ...
        public void h264_idct_add16(byte[] dst /*align 16*/, int dst_offset, int[] blockoffset, int blockoffset_offset,
            short[] block /*align 16*/, int block_offset, int stride, int[] nnzc)
        {
            int i;
            for (i = 0; i < 16; i++)
            {
                ...
                if (nnz != 0)
                {
                    if (nnz == 1 && block[i * 16] != 0)
                        h264_idct_dc_add(dst, dst_offset + blockoffset[blockoffset_offset + i], block,
                            block_offset + i * 16, stride);
                    else
                        idct_internal(dst, dst_offset + blockoffset[blockoffset_offset + i], block,
                            block_offset + i * 16, stride, 4, 6, 1);
                }
            }
        }

        public void h264_idct8_add4(byte[] dst /*align 16*/, int dst_offset, int[] blockoffset, int blockoffset_offset,
            short[] block /*align 16*/, int block_offset, int stride, int[] nnzc)
        {
            int i;
            for (i = 0; i < 16; i += 4)
            {
                ...
                {
                    if (nnz == 1 && block[i * 16] != 0)
                        h264_idct8_dc_add(dst, dst_offset + blockoffset[blockoffset_offset + i], block,
                            block_offset + i * 16, stride);
                    else
                        h264_idct8_add(dst, dst_offset + blockoffset[blockoffset_offset + i], block,
                            block_offset + i * 16, stride);
                }
            }
        }

        public void h264_idct_add8(byte[][] dst /*align 16*/, int[] dst_offset, int[] blockoffset,
            int blockoffset_offset, short[] block /*align 16*/, int block_offset, int stride, int[] nnzc)
        {
            int i;
            for (i = 16; i < 16 + 8; i++)
            {
                if (nnzc[scan8[i]] != 0)
                    idct_internal(dst[(i & 4) >> 2], dst_offset[(i & 4) >> 2] + blockoffset[blockoffset_offset + i],
                        block, block_offset + i * 16, stride, 4, 6, 1);
                else if (block[i * 16] != 0)
                    h264_idct_dc_add(dst[(i & 4) >> 2], dst_offset[(i & 4) >> 2] + blockoffset[blockoffset_offset + i],
                        block, block_offset + i * 16, stride);
            }
        }

        public void h264_idct_add16intra(byte[] dst /*align 16*/, int dst_offset, int[] blockoffset,
            int blockoffset_offset, short[] block /*align 16*/, int block_offset, int stride, int[] nnzc)
        {
            int i;
            for (i = 0; i < 16; i++)
            {
                if (nnzc[scan8[i]] != 0)
                    idct_internal(dst, dst_offset + blockoffset[blockoffset_offset + i], block, block_offset + i * 16,
                        stride, 4, 6, 1);
                else if (block[i * 16] != 0)
                    h264_idct_dc_add(dst, dst_offset + blockoffset[blockoffset_offset + i], block,
                        block_offset + i * 16, stride);
        ...
        public void h264_luma_dc_dequant_idct(short[] output, int output_offset, short[] input /*align 16*/,
            int input_offset, int qmul)
        {
            int i;
            int[] temp = new int[16];
        ...
        public void h264_chroma_dc_dequant_idct(short[] output, int output_offset, short[] input /*align 16*/,
            int input_offset, int qmul)
        {
            int stride = 16 * 2;
            int xStride = 16;
