...
namespace LightCodec.h264.decoder
{
    public class MpegEncContext
    {
        ...
        public const int VP_START = 1;

        ///< current MB is the first after a resync marker
        public const int AC_ERROR = 2;

        public const int DC_ERROR = 4;
        public const int MV_ERROR = 8;
        public const int AC_END = 16;
        public const int DC_END = 32;
        public const int MV_END = 64;
        ...
        */
        public const int CODEC_FLAG_QSCALE = 0x0002;

        ///< Use fixed qscale.
        public const int CODEC_FLAG_4MV = 0x0004;

        ///< 4 MV per MB allowed / advanced prediction for H.263.
        public const int CODEC_FLAG_QPEL = 0x0010;
        ...
        public const int CODEC_FLAG_PART = 0x0080;
        ...
        public const int CODEC_FLAG_PASS1 = 0x0200;

        ///< Use internal 2pass ratecontrol in first pass mode.
        public const int CODEC_FLAG_PASS2 = 0x0400;

        ///< Use internal 2pass ratecontrol in second pass mode.
        public const int CODEC_FLAG_EXTERN_HUFF = 0x01000;

        ///< Use external Huffman table (for MJPEG).
        public const int CODEC_FLAG_GRAY = 0x02000;

        ///< Only decode/encode grayscale.
        public const int CODEC_FLAG_EMU_EDGE = 0x04000;

        ///< Don't draw edges.
        public const int CODEC_FLAG_PSNR = 0x08000;

        ///< error[?] variables will be set during encoding.
        public const int CODEC_FLAG_TRUNCATED = 0x00010000; /** Input bitstream might be truncated at a random
														  location instead of only at frame boundaries. */

        public const int CODEC_FLAG_NORMALIZE_AQP = 0x00020000;

        ///< Normalize adaptive quantization.
        public const int CODEC_FLAG_INTERLACED_DCT = 0x00040000;

        ///< Use interlaced DCT.
        public const int CODEC_FLAG_LOW_DELAY = 0x00080000;

        ///< Force low delay.
        public const int CODEC_FLAG_ALT_SCAN = 0x00100000;

        ///< Use alternate scan.
        public const int CODEC_FLAG_GLOBAL_HEADER = 0x00400000;

        ///< Place global headers in extradata instead of every keyframe.
        public const int CODEC_FLAG_BITEXACT = 0x00800000;

        ///< Use only bitexact stuff (except (I)DCT).
        /* Fx : Flag for h263+ extra options */
        public const int CODEC_FLAG_AC_PRED = 0x01000000;

        ///< H.263 advanced intra coding / MPEG-4 AC prediction
        public const int CODEC_FLAG_H263P_UMV = 0x02000000;

        ///< unlimited motion vector
        public const int CODEC_FLAG_CBP_RD = 0x04000000;

        ///< Use rate distortion optimization for cbp.
        public const int CODEC_FLAG_QP_RD = 0x08000000;

        ///< Use rate distortion optimization for qp selectioon.
        public const int CODEC_FLAG_H263P_AIV = 0x00000008;

        ///< H.263 alternative inter VLC
        public const int CODEC_FLAG_OBMC = 0x00000001;

        ///< OBMC
        public const int CODEC_FLAG_LOOP_FILTER = 0x00000800;
        ...
        public const int CODEC_FLAG2_STRICT_GOP = 0x00000002;

        ///< Strictly enforce GOP size.
        public const int CODEC_FLAG2_NO_OUTPUT = 0x00000004;

        ///< Skip bitstream encoding.
        public const int CODEC_FLAG2_LOCAL_HEADER = 0x00000008;

        ///< Place global headers at every keyframe instead of in extradata.
        public const int CODEC_FLAG2_BPYRAMID = 0x00000010;

        ///< H.264 allow B-frames to be used as references.
        public const int CODEC_FLAG2_WPRED = 0x00000020;

        ///< H.264 weighted biprediction for B-frames
        public const int CODEC_FLAG2_MIXED_REFS = 0x00000040;

        ///< H.264 one reference per partition, as opposed to one reference per macroblock
        public const int CODEC_FLAG2_8X8DCT = 0x00000080;

        ///< H.264 high profile 8x8 transform
        public const int CODEC_FLAG2_FASTPSKIP = 0x00000100;

        ///< H.264 fast pskip
        public const int CODEC_FLAG2_AUD = 0x00000200;

        ///< H.264 access unit delimiters
        public const int CODEC_FLAG2_BRDO = 0x00000400;

        ///< B-frame rate-distortion optimization
        public const int CODEC_FLAG2_INTRA_VLC = 0x00000800;

        ///< Use MPEG-2 intra VLC table.
        public const int CODEC_FLAG2_MEMC_ONLY = 0x00001000;

        ///< Only do ME/MC (I frames -> ref, P frame -> ME+MC).
        public const int CODEC_FLAG2_DROP_FRAME_TIMECODE = 0x00002000;

        ///< timecode is in drop frame format.
        public const int CODEC_FLAG2_SKIP_RD = 0x00004000;

        ///< RD optimal MB level residual skipping
        public const int CODEC_FLAG2_CHUNKS = 0x00008000;

        ///< Input bitstream might be truncated at a packet boundaries instead of only at frame boundaries.
        public const int CODEC_FLAG2_NON_LINEAR_QUANT = 0x00010000;

        ///< Use MPEG-2 nonlinear quantizer.
        public const int CODEC_FLAG2_BIT_RESERVOIR = 0x00020000;

        ///< Use a bit reservoir when encoding if possible
        public const int CODEC_FLAG2_MBTREE = 0x00040000;

        ///< Use macroblock tree ratecontrol (x264 only)
        public const int CODEC_FLAG2_PSY = 0x00080000;

        ///< Use psycho visual optimizations.
        public const int CODEC_FLAG2_SSIM = 0x00100000;

        ///< Compute SSIM during encoding, error[] values are undefined.
        public const int CODEC_FLAG2_INTRA_REFRESH = 0x00200000;
        ...
        public const int PIX_FMT_YUV420P = 1;

        ///< planar YUV 4:2:0 = 0; 12bpp = 0; (1 Cr & Cb sample per 2x2 Y samples)
        public const int PIX_FMT_YUYV422 = 2;

        ///< packed YUV 4:2:2 = 0; 16bpp = 0; Y0 Cb Y1 Cr
        public const int PIX_FMT_RGB24 = 3;

        ///< packed RGB 8:8:8 = 0; 24bpp = 0; RGBRGB...
        public const int PIX_FMT_BGR24 = 4;

        ///< packed RGB 8:8:8 = 0; 24bpp = 0; BGRBGR...
        public const int PIX_FMT_YUV422P = 5;

        ///< planar YUV 4:2:2 = 0; 16bpp = 0; (1 Cr & Cb sample per 2x1 Y samples)
        public const int PIX_FMT_YUV444P = 6;

        ///< planar YUV 4:4:4 = 0; 24bpp = 0; (1 Cr & Cb sample per 1x1 Y samples)
        public const int PIX_FMT_YUV410P = 7;

        ///< planar YUV 4:1:0 = 0;  9bpp = 0; (1 Cr & Cb sample per 4x4 Y samples)
        public const int PIX_FMT_YUV411P = 8;

        ///< planar YUV 4:1:1 = 0; 12bpp = 0; (1 Cr & Cb sample per 4x1 Y samples)
        public const int PIX_FMT_GRAY8 = 9;

        ///<        Y         = 0;  8bpp
        public const int PIX_FMT_MONOWHITE = 10;

        ///<        Y         = 0;  1bpp = 0; 0 is white = 0; 1 is black = 0; in each byte pixels are ordered from the msb to the lsb
        public const int PIX_FMT_MONOBLACK = 11;

        ///<        Y         = 0;  1bpp = 0; 0 is black = 0; 1 is white = 0; in each byte pixels are ordered from the msb to the lsb
        public const int PIX_FMT_PAL8 = 12;

        ///< 8 bit with public const int PIX_FMT_RGB32 palette
        public const int PIX_FMT_YUVJ420P = 13;

        ///< planar YUV 4:2:0 = 0; 12bpp = 0; full scale (JPEG) = 0; deprecated in favor of public const int PIX_FMT_YUV420P and setting color_range
        public const int PIX_FMT_YUVJ422P = 14;

        ///< planar YUV 4:2:2 = 0; 16bpp = 0; full scale (JPEG) = 0; deprecated in favor of public const int PIX_FMT_YUV422P and setting color_range
        public const int PIX_FMT_YUVJ444P = 15;

        ///< planar YUV 4:4:4 = 0; 24bpp = 0; full scale (JPEG) = 0; deprecated in favor of public const int PIX_FMT_YUV444P and setting color_range
        public const int PIX_FMT_XVMC_MPEG2_MC = 16;

        ///< XVideo Motion Acceleration via common packet passing
        public const int PIX_FMT_XVMC_MPEG2_IDCT = 17;

        public const int PIX_FMT_UYVY422 = 18;

        ///< packed YUV 4:2:2 = 0; 16bpp = 0; Cb Y0 Cr Y1
        public const int PIX_FMT_UYYVYY411 = 19;

        ///< packed YUV 4:1:1 = 0; 12bpp = 0; Cb Y0 Y1 Cr Y2 Y3
        public const int PIX_FMT_BGR8 = 20;

        ///< packed RGB 3:3:2 = 0;  8bpp = 0; (msb)2B 3G 3R(lsb)
        public const int PIX_FMT_BGR4 = 21;

        ///< packed RGB 1:2:1 bitstream = 0;  4bpp = 0; (msb)1B 2G 1R(lsb) = 0; a byte contains two pixels = 0; the first pixel in the byte is the one composed by the 4 msb bits
        public const int PIX_FMT_BGR4_BYTE = 22;

        ///< packed RGB 1:2:1 = 0;  8bpp = 0; (msb)1B 2G 1R(lsb)
        public const int PIX_FMT_RGB8 = 23;

        ///< packed RGB 3:3:2 = 0;  8bpp = 0; (msb)2R 3G 3B(lsb)
        public const int PIX_FMT_RGB4 = 24;

        ///< packed RGB 1:2:1 bitstream = 0;  4bpp = 0; (msb)1R 2G 1B(lsb) = 0; a byte contains two pixels = 0; the first pixel in the byte is the one composed by the 4 msb bits
        public const int PIX_FMT_RGB4_BYTE = 25;

        ///< packed RGB 1:2:1 = 0;  8bpp = 0; (msb)1R 2G 1B(lsb)
        public const int PIX_FMT_NV12 = 26;

        ///< planar YUV 4:2:0 = 0; 12bpp = 0; 1 plane for Y and 1 plane for the UV components = 0; which are interleaved (first byte U and the following byte V)
        public const int PIX_FMT_NV21 = 27;

        ///< as above = 0; but U and V bytes are swapped
        public const int PIX_FMT_ARGB = 28;

        ///< packed ARGB 8:8:8:8 = 0; 32bpp = 0; ARGBARGB...
        public const int PIX_FMT_RGBA = 29;

        ///< packed RGBA 8:8:8:8 = 0; 32bpp = 0; RGBARGBA...
        public const int PIX_FMT_ABGR = 30;

        ///< packed ABGR 8:8:8:8 = 0; 32bpp = 0; ABGRABGR...
        public const int PIX_FMT_BGRA = 31;

        ///< packed BGRA 8:8:8:8 = 0; 32bpp = 0; BGRABGRA...
        public const int PIX_FMT_GRAY16BE = 32;

        ///<        Y         = 0; 16bpp = 0; big-endian
        public const int PIX_FMT_GRAY16LE = 33;

        ///<        Y         = 0; 16bpp = 0; little-endian
        public const int PIX_FMT_YUV440P = 34;

        ///< planar YUV 4:4:0 (1 Cr & Cb sample per 1x2 Y samples)
        public const int PIX_FMT_YUVJ440P = 35;

        ///< planar YUV 4:4:0 full scale (JPEG) = 0; deprecated in favor of public const int PIX_FMT_YUV440P and setting color_range
        public const int PIX_FMT_YUVA420P = 36;

        ///< planar YUV 4:2:0 = 0; 20bpp = 0; (1 Cr & Cb sample per 2x2 Y & A samples)
        public const int PIX_FMT_VDPAU_H264 = 37;

        ///< H.264 HW decoding with VDPAU = 0; data[0] contains a vdpau_render_state struct which contains the bitstream of the slices as well as various fields extracted from headers
        public const int PIX_FMT_VDPAU_MPEG1 = 38;

        ///< MPEG-1 HW decoding with VDPAU = 0; data[0] contains a vdpau_render_state struct which contains the bitstream of the slices as well as various fields extracted from headers
        public const int PIX_FMT_VDPAU_MPEG2 = 39;

        ///< MPEG-2 HW decoding with VDPAU = 0; data[0] contains a vdpau_render_state struct which contains the bitstream of the slices as well as various fields extracted from headers
        public const int PIX_FMT_VDPAU_WMV3 = 40;

        ///< WMV3 HW decoding with VDPAU = 0; data[0] contains a vdpau_render_state struct which contains the bitstream of the slices as well as various fields extracted from headers
        public const int PIX_FMT_VDPAU_VC1 = 41;

        ///< VC-1 HW decoding with VDPAU = 0; data[0] contains a vdpau_render_state struct which contains the bitstream of the slices as well as various fields extracted from headers
        public const int PIX_FMT_RGB48BE = 42;

        ///< packed RGB 16:16:16 = 0; 48bpp = 0; 16R = 0; 16G = 0; 16B = 0; the 2-byte value for each R/G/B component is stored as big-endian
        public const int PIX_FMT_RGB48LE = 43;

        ///< packed RGB 16:16:16 = 0; 48bpp = 0; 16R = 0; 16G = 0; 16B = 0; the 2-byte value for each R/G/B component is stored as little-endian
        public const int PIX_FMT_RGB565BE = 44;

        ///< packed RGB 5:6:5 = 0; 16bpp = 0; (msb)   5R 6G 5B(lsb) = 0; big-endian
        public const int PIX_FMT_RGB565LE = 45;

        ///< packed RGB 5:6:5 = 0; 16bpp = 0; (msb)   5R 6G 5B(lsb) = 0; little-endian
        public const int PIX_FMT_RGB555BE = 46;

        ///< packed RGB 5:5:5 = 0; 16bpp = 0; (msb)1A 5R 5G 5B(lsb) = 0; big-endian = 0; most significant bit to 0
        public const int PIX_FMT_RGB555LE = 47;

        ///< packed RGB 5:5:5 = 0; 16bpp = 0; (msb)1A 5R 5G 5B(lsb) = 0; little-endian = 0; most significant bit to 0
        public const int PIX_FMT_BGR565BE = 48;

        ///< packed BGR 5:6:5 = 0; 16bpp = 0; (msb)   5B 6G 5R(lsb) = 0; big-endian
        public const int PIX_FMT_BGR565LE = 49;

        ///< packed BGR 5:6:5 = 0; 16bpp = 0; (msb)   5B 6G 5R(lsb) = 0; little-endian
        public const int PIX_FMT_BGR555BE = 50;

        ///< packed BGR 5:5:5 = 0; 16bpp = 0; (msb)1A 5B 5G 5R(lsb) = 0; big-endian = 0; most significant bit to 1
        public const int PIX_FMT_BGR555LE = 51;

        ///< packed BGR 5:5:5 = 0; 16bpp = 0; (msb)1A 5B 5G 5R(lsb) = 0; little-endian = 0; most significant bit to 1
        public const int PIX_FMT_VAAPI_MOCO = 52;

        ///< HW acceleration through VA API at motion compensation entry-point = 0; Picture.data[3] contains a vaapi_render_state struct which contains macroblocks as well as various fields extracted from headers
        public const int PIX_FMT_VAAPI_IDCT = 53;

        ///< HW acceleration through VA API at IDCT entry-point = 0; Picture.data[3] contains a vaapi_render_state struct which contains fields extracted from headers
        public const int PIX_FMT_VAAPI_VLD = 54;

        ///< HW decoding through VA API = 0; Picture.data[3] contains a vaapi_render_state struct which contains the bitstream of the slices as well as various fields extracted from headers
        public const int PIX_FMT_YUV420P16LE = 55;

        ///< planar YUV 4:2:0 = 0; 24bpp = 0; (1 Cr & Cb sample per 2x2 Y samples) = 0; little-endian
        public const int PIX_FMT_YUV420P16BE = 56;

        ///< planar YUV 4:2:0 = 0; 24bpp = 0; (1 Cr & Cb sample per 2x2 Y samples) = 0; big-endian
        public const int PIX_FMT_YUV422P16LE = 57;

        ///< planar YUV 4:2:2 = 0; 32bpp = 0; (1 Cr & Cb sample per 2x1 Y samples) = 0; little-endian
        public const int PIX_FMT_YUV422P16BE = 58;

        ///< planar YUV 4:2:2 = 0; 32bpp = 0; (1 Cr & Cb sample per 2x1 Y samples) = 0; big-endian
        public const int PIX_FMT_YUV444P16LE = 59;

        ///< planar YUV 4:4:4 = 0; 48bpp = 0; (1 Cr & Cb sample per 1x1 Y samples) = 0; little-endian
        public const int PIX_FMT_YUV444P16BE = 60;

        ///< planar YUV 4:4:4 = 0; 48bpp = 0; (1 Cr & Cb sample per 1x1 Y samples) = 0; big-endian
        public const int PIX_FMT_VDPAU_MPEG4 = 61;

        ///< MPEG4 HW decoding with VDPAU = 0; data[0] contains a vdpau_render_state struct which contains the bitstream of the slices as well as various fields extracted from headers
        public const int PIX_FMT_DXVA2_VLD = 62;

        ///< HW decoding through DXVA2 = 0; Picture.data[3] contains a LPDIRECT3DSURFACE9 pointer
        public const int PIX_FMT_RGB444BE = 63;

        ///< packed RGB 4:4:4 = 0; 16bpp = 0; (msb)4A 4R 4G 4B(lsb) = 0; big-endian = 0; most significant bits to 0
        public const int PIX_FMT_RGB444LE = 64;

        ///< packed RGB 4:4:4 = 0; 16bpp = 0; (msb)4A 4R 4G 4B(lsb) = 0; little-endian = 0; most significant bits to 0
        public const int PIX_FMT_BGR444BE = 65;

        ///< packed BGR 4:4:4 = 0; 16bpp = 0; (msb)4A 4B 4G 4R(lsb) = 0; big-endian = 0; most significant bits to 1
        public const int PIX_FMT_BGR444LE = 66;

        ///< packed BGR 4:4:4 = 0; 16bpp = 0; (msb)4A 4B 4G 4R(lsb) = 0; little-endian = 0; most significant bits to 1
        public const int PIX_FMT_Y400A = 67;

        ///< 8bit gray = 0; 8bit alpha
        public const int PIX_FMT_NB = 68;

        ///< number of pixel formats = 0; DO NOT USE THIS if you want to link with shared libav* because the number of formats might differ between versions
        //};
        ...
        public const int INTERNAL_BUFFER_SIZE = 32;
        public const int STRIDE_ALIGN = 8;
        ...
        public const int CONFIG_GRAY = 0;

        public const int MAX_PICTURE_COUNT = 32;

        public const int FMT_H264 = 4;
        ...
        */
        ...
        public int h_edge_pos, v_edge_pos;
        ...
        */
        ...
        public const int PREV_PICT_TYPES_BUFFER_SIZE = 256;
        ...
        public /*uint8_t*/ int[] rd_scratchpad;

        ///< scratchpad for rate distortion mb decision
        public /*uint8_t*/ int obmc_scratchpad_offset;

        public /*uint8_t*/ int[] b_scratchpad;
        ...
        public /*&uint8_t **/ int[] error_status_table;
        ...
        */
        ...
        public static int av_image_check_size( /*unsigned*/ int w, /*unsigned*/ int h, int log_offset, /*void **/
        ...
        public int ff_dct_common_init()
        {
            ...
            */
            ...
            {
                ff_init_scantable(this.dsp.idct_permutation, this.inter_scantable,
                    H264Context.ff_alternate_vertical_scan);
                ff_init_scantable(this.dsp.idct_permutation, this.intra_scantable,
                    H264Context.ff_alternate_vertical_scan);
            }
            else
            {
            ...
            ff_init_scantable(this.dsp.idct_permutation, this.intra_h_scantable,
                H264Context.ff_alternate_horizontal_scan);
            ff_init_scantable(this.dsp.idct_permutation, this.intra_v_scantable,
                H264Context.ff_alternate_vertical_scan);
        ...
        public int ff_set_systematic_pal2(byte[] pal /*[256]*/, int pal_offset, int pix_fmt)
        {
            ...
            for (i = 0; i < 256; i++)
            {
                ...
                //pal[i] = b + (g<<8) + (r<<16);
                pal[pal_offset + i * 4 + 0] = (byte)Math.Clamp(b, 0, 255);
                pal[pal_offset + i * 4 + 1] = (byte)Math.Clamp(g, 0, 255);
                pal[pal_offset + i * 4 + 2] = (byte)Math.Clamp(r, 0, 255);
                pal[pal_offset + i * 4 + 3] = (byte)Math.Clamp(0x00, 0, 255); // a?	        
            }
        ...
        public int get_buffer(AVFrame pic)
        {
            ...
            else
            {
                ...
                tmpsize = av_image_fill_pointers(picture.data_base, picture.data_offset, this.pix_fmt, h, null,
                    picture.linesize);
                if (tmpsize < 0)
                {
        ...
        {
            int big_mb_num =
                this.mb_stride * (this.mb_height + 1) + 1; //the +1 is needed so memset(,,stride*height) does not sig11
        ...
        public int MPV_frame_start()
        {
            ...
            */
            ...
            if (true)
            {
                ...
                for (i = 0; i < MAX_PICTURE_COUNT; i++)
                {
                    if (this.picture[i].data_base[0] != null && 0 == this.picture[i].reference &&
                        this.picture[i].type != FF_BUFFER_TYPE_SHARED)
                    {
                ...
                if (this.current_picture_ptr != null && this.current_picture_ptr.data_base[0] == null)
                {
                    ...
                    pic = this
                        .current_picture_ptr; //we already have a unused image (maybe it was set before reading the header)
                }
                else
                {
        ...
        public int MPV_common_init()
        {
            ...
            this.mb_index2xy[this.mb_height * this.mb_width] =
                (this.mb_height - 1) * this.mb_stride + this.mb_width; //FIXME really needed?
...
#pragma warning restore CS8600
        ...
        public void MPV_frame_end()
        {
            ...
            {
                this.dsp.draw_edges(this.current_picture.data_base[0], this.current_picture.data_offset[0],
                    this.linesize, this.h_edge_pos, this.v_edge_pos, EDGE_WIDTH);
                this.dsp.draw_edges(this.current_picture.data_base[1], this.current_picture.data_offset[1],
                    this.uvlinesize, this.h_edge_pos >> 1, this.v_edge_pos >> 1, EDGE_WIDTH / 2);
                this.dsp.draw_edges(this.current_picture.data_base[2], this.current_picture.data_offset[2],
                    this.uvlinesize, this.h_edge_pos >> 1, this.v_edge_pos >> 1, EDGE_WIDTH / 2);
            }
        ...
        public void MPV_common_defaults()
        {
            ...
            this.picture_structure = Constants.PICT_FRAME;
        ...
        public static MpegEncContext init()
        {
        ...
        public int open(H264Decoder codec)
        {
            int ret = -1;
        ...
        public int avdecode(AVFrame picture, int[] got_picture_ptr /* [0] = in/out param */,
            AVPacket avpkt)
        {
            int ret;
